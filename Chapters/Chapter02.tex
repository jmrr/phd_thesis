...
\begin{figure}
\centering
\includegraphics[height=6cm]{./gfx/Chapter02/pathexample2.pdf}
...
The $\gamma$ measure is applied between pairs of query and database images, and one may identify two types of categories that these query comparisons fall into.  In the first case, the images come from the same path (although query and visual path database are, of course, distinct).  In the other case, queries come from different paths.  The results are shown in Fig.~\ref{fig:rhoAndGammaDistributions}(a).   

\begin{figure*}[t]
\begin{center}
\subfloat[Distributions for the $\gamma$ metric.] {\includegraphics[width=0.48\linewidth]{./gfx/Chapter02/path_pdf_analysisWithShadedBetweensALLDB.pdf}}
\subfloat[Distributions for the $\rho$ metric.]
{\includegraphics[width=0.48\linewidth]{./gfx/Chapter02/C5distributions_no_smoothingWithSmoothedHistograms-latex.pdf}}
\caption{Tests of visual distinctiveness along paths.  (a) path level queries, capturing the behaviour of the $\gamma$ metric for inter- and intra-path distributions and (b) locations within a path, illustrating the distribution trends of the $\rho$-metric, all within a single 80m path, but at different distances either within or outside 50cm from known query submissions.}        
\label{fig:rhoAndGammaDistributions}
\end{center}
\end{figure*}
...
For the analysis of the distribution of the metrics $\gamma$ and $\rho$, as shown in Fig.~\ref{fig:rhoAndGammaDistributions}, we have used VLFEAT's~\cite{Vedaldi2008} implementation of SIFT~\cite{Lowe2004} descriptors.
...
\begin{table}
\begin{center}
\begin{tabularx}{1.1\linewidth}{XXXXXXXXXXXX}
\toprule
