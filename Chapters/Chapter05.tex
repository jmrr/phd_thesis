...
\begin{quotation}{\it Given a video sequence taken by a person as they walk, can we generate artificial place cell responses that are able to localise that person with respect to previous journeys?}
\end{quotation}
...
One way to mimic the behaviour of BPCs within APCs becomes patent when we revisit the {\it kernel} function we introduced in the previous chapter. This {\ite kernel} maps a pair of FEVs onto a positive scalar value. We used the following mapping between two vectors $\mathbf{v}_a$ and $\mathbf{v}_b$:
\be
