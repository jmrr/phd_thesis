...
This computational framework enhances the approach taken in Chapter~\ref{ch:chapter4} -- in which localisation was based on the location of the best match -- with a population coding approach: a population of artificial place cells. 
...
%\end{equation}
%The pooling tensor is also an order 3 tensor, that defines 17 pooling regions with respect to each location in image space, distributed in a radial and angular fashion across a patch; the values in this tensor are visualised over a normalised neighbourhood of unit width and height in Figure~\ref{fig:PoolerResponses}. The resulting order 4 tensor, $\tens{D}$, may be reshaped \citep{kolda2009tensor} into an order 3 tensor containing 136 slices along mode $i_3$. Descriptors are obtained by sampling $\tens{D}$ every 3 pixels along modes $i_1$ and $i_2$, generating around 2,000 descriptor vectors per frame, each of 136 elements.
%
%Both the pooling patterns and the Gabor parameters $\sigma$ and $\lambda$ were optimised on the PASCAL VOC 2007 database \citep{everingham2010pascal} for retrieval accuracy. No further optimisation was applied for the experiments on the RSM dataset as described in this chapter.
%
%
%%%
...
\label{ch05:overview}
In the experimental work to be discussed in the next section, a Generalised Regression Neural Network was used to provide sub-APC position estimates, obviating the need to construct {\em ad-hoc} empirical estimators.  This regression network consists of two-layers, and uses radial-basis functions.  The responses from $C = 16$ place cells were input to the network, and ground truth of location within a section of corridor -- up to 4 m long -- used to train it as a regressor.  In all experiments, dictionary generation was performed independently of the APC responses used in training the regression network. 
...
\caption{Overview of the training pipeline. The diagram of the neural network is merely illustrative, it does not represent the real architecture used.\td{Consider editing it}{Perhaps change the modules from gradients to kernels for FEV construction}{}}
\label{fig:pipeline}
\end{figure}
