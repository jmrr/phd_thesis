...
Modes that do not participate in the convolution or the permutation must have equal dimension. Finally, where a third mode is implicit in one of the arguments of the permuted indices (such as for an order 2 tensor that is being permuted with an order 3 tensor), this is indicated by a ``dot'' symbol (e.g. $\mathcal{P} = \lbrace (\cdot,3))$.

\noindent{\textit{Remark}} The modes of permutation may be thought of as describing the scaffolding of a convolutional network, and is related to the topology of a multi-layered network \cite{chang2015deep}. 
