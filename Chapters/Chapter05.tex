...
This GRNN network consists of two layers: the first layer has radial basis functions (RBF) neurons with a spread of 0.1. The second is a linear output layer that calculates the position estimates from the RBF outputs. This supervised phase of training used a subset of videos to infer spatial location from the ground truth camera position of this subset of the dataset.  The trained network was then applied to infer the spatial position in videos from the remainder of the dataset. In the experiments, the responses from $C = 16$ place cells were input to the network, and ground truth of location within a section of corridor -- up to 4 m long -- used to train the network.  In all experiments, dictionary generation was performed independently of the APC responses used in training the regression network. 
